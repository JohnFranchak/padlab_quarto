...
presentations %>% filter(type %in% c("Colloquium")) %>% 
  mutate(citation = sprintf("%d. (%d) %s.", row_number(), year, venue)) %>% 
  select(citation) %>% kable(col.names = "")
```
:::
::: {.content-visible when-format="html"}
...
| Simon’s Foundation Autism Research Initiative. (2011) [*Eye-tracking device travels with toddlers*](http://sfari.org/news-and-opinion/toolbox/2011/eye-tracking-device-travels-with-toddlers) 
