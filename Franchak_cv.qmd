...
### Extramural
 | 
|--|--------|
| 2022-2025 | James S. McDonnell Foundation Opportunity Award: [*Characterizing infants' everyday motor and object experiences through computer vision analysis of caregiver-captured video surveys.*](https://doi.org/10.37717/2022-3803) **John Franchak, PI, $250,000 3-year project.** |
...
::: {.content-visible when-format="pdf"}
```{r}
presentations %>% filter(type %in% c("Paper")) %>% 
  mutate(citation = sprintf("%d.  %s (%d, %s). *%s*. %s %s.", row_number(), authors, year, month, title, what, venue)) %>% 
  select(citation) %>% kable(col.names = "")
```
:::
...
::: {.content-visible when-format="pdf"}
```{r}
presentations %>% filter(type %in% c("Poster")) %>% 
  mutate(citation = sprintf("%d.  %s (%d, %s). *%s*. %s %s.", row_number(), authors, year, month, title, what, venue)) %>% 
  select(citation) %>% kable(col.names = "")
```
:::
...
::: {.content-visible when-format="pdf"}
```{r}
presentations %>% filter(type %in% c("Colloquium")) %>% 
  mutate(citation = sprintf("%d. %s. (%d)", row_number(), venue, year)) %>% 
  select(citation) %>% kable(col.names = "")
```
:::
::: {.content-visible when-format="html"}
```{r}
presentations %>% filter(type %in% c("Colloquium")) %>% 
  mutate(citation = sprintf("(%d) %s.", year, venue)) %>% 
  select(citation) %>% kable(col.names = "")
```
:::
\renewcommand{\arraystretch}{1.5}
...
| 2020- | Review Editor, Perception Science Editorial Board, *Frontiers in Psychology* and *Frontiers in Neuroscience* |
...
| **Undergraduate:** 
| PSYC 12: Psychological Methods: Research Procedures (W15, F15, F16, F17, S19, S20, U20, F20, U21, F21, U22, S23) 
| PSYC 166A: Infancy (F19, S23) 
| PSYC 169: Seminar: Perceptual-Motor Development (F14) 
| PSYC 169: Seminar: Infant Development (S17) 

| **Graduate:** 
| PSYC 207C: Processes of Cognitive Development (S16, S18, S21, S22) 
...
| 2019- | Member, UC Riverside Academic Integrity Committee |
| 2019- | Psychology Department Colloquium Series Organizer |
| 2018 | Judge, Undergraduate Research Symposium |
| 2017- | Member, Undergraduate Minigrant Review Committee |
